...
request_to_api_box(){
        ...
        while [ $do_curl -eq 1 ]; do
                ...
                case $task in
                        ...
                        ;;
                        "LIST")
                                 #http_response_code=$(curl -w "%{http_code}" -o $temp_path/folder_$folder_id -s -X GET "https://api.box.com/2.0/folders/$folder_id/items?limit=$box_search_limit_result" \
...
create_metadata_on_file() {
        ...
        echo "}" >>  $metadata_box_json
                ...
                echo $(date) "file $file_id is not an picture " >> $insert_metadata_logfile
        fi
...
#       cat $temp_path/folder_$folder_id | jq -r '.entries[] | "\(.type);\(.id);\(.name)"'
...
while [ $( pgrep -P $current_parent_pid | grep -v $refresh_token_loop_pid | wc -l ) -gt 0 ];do
        last_childs_number=$(pgrep -P $current_parent_pid | grep -v $refresh_token_loop_pid | wc -l )
        echo $(date) "Waiting for $last_childs_number child processes to finish" >> $insert_metadata_logfile
        sleep 6
done

kill $refresh_token_loop_pid
wait $refresh_token_loop_pid
